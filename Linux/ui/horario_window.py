...
class HorarioWindow(Gtk.Box):
    def __init__(self, app, session, run_async_callback, on_back):
        ...
        # Lateral: Seleção de dia da semana
        self.sidebar = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        self.sidebar.set_size_request(140, -1)
        
        chip_scroll = Gtk.ScrolledWindow()
        chip_scroll.set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.AUTOMATIC)
        chip_scroll.set_vexpand(True)
        chip_scroll.set_margin_start(8)
        chip_scroll.set_margin_end(8)
        chip_scroll.set_margin_top(8)
        chip_scroll.set_margin_bottom(8)
        self.sidebar.append(chip_scroll)

        self.chip_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6)
        chip_scroll.set_child(self.chip_box)

        self.main_content.append(self.sidebar)

        # Divisor Master-Detail
        separator = Gtk.Separator(orientation=Gtk.Orientation.VERTICAL)
        self.main_content.append(separator)

        # Painel da Direita: Lista de aulas no Stack
        self.detail_panel = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        ...
        self.detail_panel.append(self.stack)

        self.main_content.append(self.detail_panel)

        # Scroll para alternar os dias (adicionado na pilha)
        scroll_ctrl = Gtk.EventControllerScroll.new(Gtk.EventControllerScrollFlags.VERTICAL)
        scroll_ctrl.connect('scroll', self._on_stack_scroll)
        self.stack.add_controller(scroll_ctrl)
    ...
    def _criar_card_aula(self, aula: Aula) -> Gtk.Box:
        ...
        # Linha 3: sala (emoji removido)
        sala_txt = aula.sala.strip() or "Sala não informada"
        sala_lbl = Gtk.Label(label=sala_txt)
    ...
    # ===================== INTERAÇÕES DE SCROLL E SELEÇÃO =====================

    def _on_stack_scroll(self, ctrl, dx, dy):
        if self._scroll_timeout:
            return True # Bloqueia disparos excessivos de scroll
        
        # dy > 0 indica rolagem para baixo (próximo dia)
        if dy > 0.1:
            self._switch_day(1)
            self._scroll_timeout = GLib.timeout_add(350, self._clear_scroll_timeout)
            return True
        # dy < 0 indica rolagem para cima (dia anterior)
        elif dy < -0.1:
            self._switch_day(-1)
            self._scroll_timeout = GLib.timeout_add(350, self._clear_scroll_timeout)
            return True
    ...
    def _clear_scroll_timeout(self):
        self._scroll_timeout = 0
        return False

    def _switch_day(self, direction):
        atual = self.stack.get_visible_child_name()
        if not atual or not self._dias_visiveis:
            return
        
        try:
            idx = self._dias_visiveis.index(atual)
            novo_idx = idx + direction
            if 0 <= novo_idx < len(self._dias_visiveis):
                self._selecionar_dia(self._dias_visiveis[novo_idx], animar=True)
        except ValueError:
            pass
    ...
    def _selecionar_dia(self, dia: str, animar: bool):
        if not animar:
            ...
            # Direção da animação vertical (já que os chips são verticais)
            ...
            self.stack.set_transition_type(
                Gtk.StackTransitionType.SLIDE_UP if idx_novo > idx_atual
                else Gtk.StackTransitionType.SLIDE_DOWN
            )

        self.stack.set_visible_child_name(dia)
        # Retorna ao padrão (caso perca a direção em toques futuros muito rápidos)
        self.stack.set_transition_type(Gtk.StackTransitionType.SLIDE_UP_DOWN)
