...
class OpenFEIApp(Gtk.Application):
    ...
    def _read_tray_output(self, proc):
        """Lê os comandos recebidos do processo independente da tray"""
        for line in proc.stdout:
            ...
            if msg == 'show':
                ...
                print(f"[TRAY LOG] {msg}") # Imprime logs de debug do tray

    def _read_tray_errors(self, proc):
        """Lê e exibe erros do processo da tray no console"""
        for line in proc.stderr:
            print(f"[TRAY ERROR] {line.strip()}", file=sys.stderr)
    ...
    def on_activate(self, app):
        ...
        if not self.tray_process:
            tray_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "tray_service.py")
            # CORREÇÃO: text=True e encoding='utf-8' resolvem o warning do bufsize=1
            # e permitem que o readline() funcione corretamente com strings.
            self.tray_process = subprocess.Popen(
                [sys.executable, tray_script],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE, # Removido DEVNULL para vermos os erros reais
                text=True,
                encoding='utf-8',
                bufsize=1
            )
            threading.Thread(target=self._read_tray_output, args=(self.tray_process,), daemon=True).start()
            threading.Thread(target=self._read_tray_errors, args=(self.tray_process,), daemon=True).start()
