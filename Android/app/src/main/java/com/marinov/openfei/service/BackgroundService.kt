...
class BackgroundService : Service() {
    ...
    private suspend fun runWithSyncNotification(notificationId: Int, block: suspend () -> Unit) {
        ...
        try { block() } finally { cancelNotification(notificationId) }
    }
    ...
    private fun showSyncNotification(notificationId: Int) {
        ...
        try { NotificationManagerCompat.from(this).notify(notificationId, notification) }
        catch (e: SecurityException) { Log.w(TAG, "Sem permissão para notificação de sync #$notificationId", e) }
    }

    private fun cancelNotification(notificationId: Int) {
        try { NotificationManagerCompat.from(this).cancel(notificationId) }
        catch (e: Exception) { Log.w(TAG, "Erro ao cancelar notificação #$notificationId", e) }
    }
    ...
    private fun sendNotasNotification(notasAlteradas: List<Nota>) {
        ...
        try { NotificationManagerCompat.from(applicationContext).notify(NOTIF_ID_NOTAS, notification) }
        catch (e: SecurityException) { Log.w(TAG, "Sem permissão para notificação de notas", e) }
    }
    ...
    private fun sendBoletosNotification() {
        ...
        try { NotificationManagerCompat.from(applicationContext).notify(NOTIF_ID_BOLETOS, notification) }
        catch (e: SecurityException) { Log.w(TAG, "Sem permissão para notificação de boletos", e) }
    }
    ...
    private fun sendHorarioNotification() {
        ...
        try { NotificationManagerCompat.from(applicationContext).notify(NOTIF_ID_HORARIO, notification) }
        catch (e: SecurityException) { Log.w(TAG, "Sem permissão para notificação de horário", e) }
    }
    ...
    private fun sendUpdateNotification(version: String) {
        ...
        try { NotificationManagerCompat.from(applicationContext).notify(NOTIF_ID_UPDATE, notification) }
        catch (e: SecurityException) { Log.w(TAG, "Sem permissão para notificação de atualização", e) }
    }
}