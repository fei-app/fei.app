...
object CalendarioRepository {
    ...
    suspend fun obterProvasFEI(online: Boolean): List<ProvaCalendario> {
        return if (online) {
            try {
                ...
                if (e !is CancellationException) Log.e(TAG, "Erro ao buscar provas FEI online", e)
                CacheHelper.getCachedProvasFEI()
            }
        } else { CacheHelper.getCachedProvasFEI() }
    }
    ...
    fun obterCalendarioProvasCache(): List<ProvaCalendario> = CacheHelper.getCachedProvasFEI()
    fun obterProvasFEICache(): List<ProvaCalendario> = CacheHelper.getCachedProvasFEI()
    ...
    suspend fun obterEventosMoodle(online: Boolean): List<ProvaCalendario> {
        return if (online) {
            try {
                ...
                if (e !is CancellationException) Log.e(TAG, "Erro ao buscar eventos Moodle online", e)
                CacheHelper.getCachedEventosMoodle()
            }
        } else { CacheHelper.getCachedEventosMoodle() }
    }

    fun obterEventosMoodleCache(): List<ProvaCalendario> = CacheHelper.getCachedEventosMoodle()
        ...
        for (panel in panels) {
            ...
            for (linha in linhas) {
                ...
                if (disciplinaElem == null || provaElem == null || horaElem == null ||
                    salaElem == null || coordenadorElem == null) continue
                ...
                if (codigo.isNotEmpty() && dataProva.isNotEmpty()) {
                    ...
                    provas.add(ProvaCalendario(codigo, nome, dataProva, hora, sala, coordenador, tipoProva))
                }
            }
        }
        return provas
    ...
    private suspend fun fetchMoodleCalendarEvents(): List<ProvaCalendario> {
        ...
        if (token == null) {
            Log.w(TAG, "Token do Moodle indisponível — pulando eventos do Moodle")
            return emptyList()
        }
        ...
        return mapMoodleEventsToProvas(events, courses)
    }
        ...
        try {
            ...
            if (jsonElement.isJsonObject) {
                ...
                if (jsonObj.has("error") && !jsonObj.get("error").isJsonNull) {
                    val errorCode = try { jsonObj.get("errorcode")?.asString ?: "" } catch (_: Exception) { "" }
    ...
    private fun mapMoodleEventsToProvas(events: List<MoodleEvent>, courses: List<MoodleCourse>): List<ProvaCalendario> {
        ...
        return events.mapNotNull { event ->
            ...
            val nomeLimpo = nomeEvento.replace("está marcado(a) para esta data", "").trim()
            val (data, hora) = formatMoodleTimestamp(event.timestart)
            ProvaCalendario(codigo, nomeLimpo, data, hora, sala, "", "Moodle")
        }
    }
}